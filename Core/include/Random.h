...
public:
  ...
  __device__ static float Float(curandState *state, float min, float max,
                                int id) {
  ...
  __device__ static glm::vec3 Vec3(curandState *state, float min, float max,
                                   int id) {
    return {Float(state, min, max, id), Float(state, min, max, id),
            Float(state, min, max, id)};
  }
  ...
  __device__ static glm::vec3 PcgInUnitSphere(uint32_t &seed) {
    return glm::normalize(glm::vec3(PcgFloat(seed) * 2.0f - 1.0f,
                                    PcgFloat(seed) * 2.0f - 1.0f,
                                    PcgFloat(seed) * 2.0f - 1.0f));
  }
  ...
  __device__ static glm::vec2 PcgInUnitDisk(uint32_t &seed) {
    glm::vec2 p;
    do {
      p = 2.0f * glm::vec2(PcgFloat(seed), PcgFloat(seed)) -
          glm::vec2(1.0f, 1.0f);
