...
static void SetupVulkan(const char **extensions, uint32_t extensions_count) {
...

    {
        ...
        const char **extensions_ext = static_cast<const char **>(
            malloc(sizeof(const char *) * (extensions_count + 1)));
...
void Application::run() {
    ...
    while (!glfwWindowShouldClose(m_window) && m_running) {
        ...
        for (auto &layer : m_layers) layer->onUpdate(m_timeStep);
        ...
        if (g_SwapChainRebuild) {
            ...
            if (width > 0 && height > 0) {
                ...
                SetupVulkanWindow(&g_MainWindowData, g_MainWindowData.Surface,
                                  width, height, m_vsyncEnabled);
...
VkCommandBuffer Application::submitSingleTimeCommands(
