  ...
  while (!glfwWindowShouldClose(m_window) && m_running) {
    ...
    if (g_SwapChainRebuild) {
      ...
      if (width > 0 && height > 0) {
        ...
        // height, g_MinImageCount);
        SetupVulkanWindow(&g_MainWindowData, g_MainWindowData.Surface, width,
                          height, m_vsyncEnabled);
...
VkCommandBuffer
Application::submitSingleTimeCommands(VkCommandBuffer commandBuffer) {
