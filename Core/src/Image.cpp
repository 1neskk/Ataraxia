...
void Image::release() {
  Application::submitResourceFree([sampler = m_sampler, imageView = m_imageView,
                                   image = m_image, memory = m_memory,
                                   stagingBuffer = m_buffer,
                                   stagingBufferMemory = m_bufferMemory]() {
    VkDevice device = Application::getDevice();
...
void Image::setData(const void *data) {
  ...
  {
    char *map = nullptr;
    err = vkMapMemory(device, m_bufferMemory, 0, m_size, 0,
