...
json Utils::serializeScene(const Scene &scene) {
    ...
    j["camera"] = {
        {"position",
         {scene.camera.getPosition().x, scene.camera.getPosition().y,
          scene.camera.getPosition().z}},
        {"direction",
         {scene.camera.getDirection().x, scene.camera.getDirection().y,
          scene.camera.getDirection().z}},
...
json Utils::serializeSceneNode(const std::shared_ptr<SceneNode> &node) {
    ...
    j["transformation"] = {
        ...
        {"rotation",
         {node->getRotation().x, node->getRotation().y, node->getRotation().z,
          node->getRotation().w}},
        {"scale",
...
Scene Utils::deserializeScene(const json &j) {
    ...
    scene.camera.setPosition(glm::vec3(j["camera"]["position"][0],
                                       j["camera"]["position"][1],
                                       j["camera"]["position"][2]));
    scene.camera.setDirection(glm::vec3(j["camera"]["direction"][0],
                                        j["camera"]["direction"][1],
                                        j["camera"]["direction"][2]));
    ...
    for (const auto &material : j["materials"]) {
        Material m;
        m.albedo = glm::vec3(material["albedo"][0], material["albedo"][1],
                             material["albedo"][2]);
        m.roughness = material["roughness"];
        m.metallic = material["metallic"];
        m.F0 =
            glm::vec3(material["F0"][0], material["F0"][1], material["F0"][2]);
        scene.materials.push_back(m);
    }

    for (const auto &light : j["lights"]) {
        Light l;
        l.position = glm::vec3(light["position"][0], light["position"][1],
                               light["position"][2]);
        l.intensity = light["intensity"];
        l.color =
            glm::vec3(light["color"][0], light["color"][1], light["color"][2]);
...
void Utils::deserializeSceneNode(const json &j,
                                 std::shared_ptr<SceneNode> &node) {
    if (!node) node = std::make_shared<SceneNode>(j["name"].get<std::string>());

    glm::vec3 position(j["transformation"]["position"][0],
                       j["transformation"]["position"][1],
                       j["transformation"]["position"][2]);
    glm::quat rotation(
        j["transformation"]["rotation"][3], j["transformation"]["rotation"][0],
        j["transformation"]["rotation"][1], j["transformation"]["rotation"][2]);
    glm::vec3 scale(j["transformation"]["scale"][0],
                    j["transformation"]["scale"][1],
                    j["transformation"]["scale"][2]);
    ...
    if (j.contains("spheres")) {
        for (const auto &sphere : j["spheres"]) {
            Sphere s;
            s.center = glm::vec3(sphere["center"][0], sphere["center"][1],
                                 sphere["center"][2]);
