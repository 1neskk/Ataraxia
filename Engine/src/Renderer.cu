...
void Renderer::allocateDeviceMemory(const Scene& scene) {
    ...
    for (auto& sphere : collectedSpheres) {
        if (static_cast<uint32_t>(sphere.materialIndex) >=
...
void Renderer::traverseSceneGraph(const std::shared_ptr<SceneNode>& node,
    ...
    if (node) {
        ...
        for (const auto& sphere : node->getSpheres()) {
            ...
            glm::vec4 transformedCenter =
                globalTransform * glm::vec4(sphere.center.x, sphere.center.y,
                                            sphere.center.z, 1.0f);
            transformedSphere.center =
                glm::vec3(transformedCenter) / transformedCenter.w;
...
void Renderer::onResize(uint32_t width, uint32_t height) {
    if (m_image && m_image->getWidth() == width &&
        m_image->getHeight() == height)
        return;
    ...
    d_accumulation_.Release();
    d_accumulation_ =
        CudaBuffer<glm::vec4>(static_cast<uint64_t>(width) * height);
...
namespace {
__global__ void kernelRender(uint32_t width, uint32_t height,
                             uint32_t* imageData, const Sphere* spheres,
                             size_t numSpheres, const BVHNode* nodes,
                             const DeviceCamera d_camera,
                             const Material* materials, size_t numMaterials,
                             glm::vec4* accumulation, uint32_t frameIndex,
                             const Light* lights, size_t numLights,
                             Settings settings) {
    if (width == 0 || height == 0 || d_camera.width == 0 ||
        d_camera.height == 0)
        return;
    ...
    if (x < width && y < height) {
        const glm::vec4 color = Renderer::perPixel(
            x, y, width, spheres, numSpheres, nodes, d_camera, materials,
            numMaterials, frameIndex, lights, numLights, settings);
        const uint32_t pixelIndex = x + y * width;
        accumulation[pixelIndex] += color;
        glm::vec4 finalColor =
            accumulation[pixelIndex] / static_cast<float>(frameIndex);
...
void Renderer::Render(Camera& camera, const Scene& scene) {
    ...
    if (m_frameIndex == 1)
        cudaMemset(
            d_accumulation_.GetData(), 0,
            static_cast<uint64_t>(m_width) * m_height * sizeof(glm::vec4));
    ...
    CUDA_CHECK(cudaOccupancyMaxPotentialBlockSize(
        &minGridSize, &optimalBlockSize, kernelRender, 0, 0));
    ...
    std::cout << "Block size: (" << blockSize.x << ", " << blockSize.y << ") | "
              << blockSize.x * blockSize.y << " Threads per block\n";
#endif
    ...
    kernelRender<<<gridSize, blockSize>>>(
        m_width, m_height, d_imageData_.GetData(), d_spheres_.GetData(),
        m_numSpheres, d_bvhNodes_.GetData(), d_camera, d_materials_.GetData(),
        m_numMaterials, d_accumulation_.GetData(), m_frameIndex,
        d_lights_.GetData(), m_numLights, m_settings);
    ...
    if (err != cudaSuccess) {
        std::cerr << "CUDA kernel synchronization error: "
                  << cudaGetErrorString(err) << "\n";
        return;
    }

    d_imageData_.CopyToHost(h_imageData_,
...
__device__ bool intersectAABB(const Ray& ray, const glm::vec3& bmin,
                              const glm::vec3& bmax, float& t) {
...
__device__ Renderer::HitRecord Renderer::traceRay(const Ray& ray,
    ...
    if (numSpheres == 0 || nodes == nullptr) return rayMiss(ray);
    ...
    while (stackPtr > 0) {
        ...
        float tBox;
        if (!intersectAABB(ray, node.aabbMin, node.aabbMax, tBox)) continue;

        if (tBox > tmin) continue;

        if (node.count > 0)  // Leaf
        {
            for (uint32_t i = 0; i < node.count; i++) {
                uint32_t sphereIdx = node.leftFirst + i;
                const auto& [center, radius, materialIndex] =
                    spheres[sphereIdx];
        ...
        } else  // Internal
        {
            ...
            float tLeft, tRight;
            bool hitLeft = intersectAABB(ray, nodes[leftIdx].aabbMin,
                                         nodes[leftIdx].aabbMax, tLeft);
            bool hitRight = intersectAABB(ray, nodes[rightIdx].aabbMin,
                                          nodes[rightIdx].aabbMax, tRight);
    ...
    if (closestSphere < 0) return rayMiss(ray);

    return rayHit(ray, tmin, closestSphere, spheres);
}

__device__ glm::vec4 Renderer::perPixel(
    uint32_t x, uint32_t y, uint32_t width, const Sphere* spheres,
    size_t numSpheres, const BVHNode* nodes, const DeviceCamera& d_camera,
    const Material* materials, size_t numMaterials, uint32_t frameIndex,
    ...
    glm::vec4 target = d_camera.inverseProjectionMatrix *
                       glm::vec4(coord.x, coord.y, 1.0f, 1.0f);
    glm::vec3 rayDir = glm::normalize(glm::vec3(
        d_camera.inverseViewMatrix *
        glm::vec4(glm::normalize(glm::vec3(target) / target.w), 0.0f)));
    ...
    // The throughput vector accounts for the attenuation of light as it bounces
    // around the scene.
    ...
    for (int i = 0; i < bounces; i++) {
        ...
        if (ht.t < 0.0f) {
            if (settings.skyLight) {
                // skyLight setting just adds a simple gradient to the
                // background. It is not an actual light source.
        ...
        glm::vec3 baseReflectivity =
            glm::mix(mat->F0, mat->albedo, mat->metallic);
        ...
        if (numLights > 0) {
            ...
            HitRecord shadowHt =
                traceRay(shadowRay, spheres, numSpheres, nodes);
            if (shadowHt.t > 0.0f &&
                shadowHt.t * shadowHt.t < distanceSquared) {
                // Light is occluded; skip contribution.
            } else {
                // Evaluate BRDF where V is the view direction, N is the surface
                // normal, and L is the light direction.
                glm::vec3 V = -ray.direction;
                glm::vec3 N = ht.worldNormal;
                glm::vec3 specular =
                    BRDF::cookTorrance(mat->albedo, baseReflectivity,
                                       mat->metallic, mat->roughness, N, V, L);
                glm::vec3 emission =
                    sampledLight.color * sampledLight.intensity;
        ...
        if (Random::Random::PcgFloat(seed) < specularProb) {
            ray.direction =
                BRDF::sampleGGX(ht.worldNormal, mat->roughness, seed);
            bounceReflectivity = baseReflectivity;
        } else {
            ray.direction =
                BRDF::sampleHemisphereCosineWeighted(ht.worldNormal, seed);
        ...
        if (Random::Random::PcgFloat(seed) > rouletteProb ||
            rouletteProb <= 0.0f)
            break;
...
__device__ Renderer::HitRecord Renderer::rayHit(const Ray& ray, float tmin,
                                                const int index,
                                                const Sphere* spheres) {
    HitRecord ht;
    ht.t = tmin;
