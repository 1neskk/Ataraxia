...
__device__ glm::vec3 BRDF::cookTorrance(const glm::vec3& albedo,
...

{
    ...
    float NdotL =
        glm::max(glm::dot(N, L), 0.0000001f);  // prevent division by zero
...
__device__ float BRDF::geometrySmith(float NdotV, float NdotL,
                                     float roughness) {
...
__device__ glm::vec3 BRDF::sampleHemisphereCosineWeighted(const glm::vec3& N,
                                                          uint32_t& seed) {
...
__device__ glm::vec3 BRDF::sampleGGX(const glm::vec3& N, float roughness,
                                     uint32_t& seed) {
    const float u1 = Random::Random::PcgFloat(seed);
    const float u2 = Random::Random::PcgFloat(seed);
