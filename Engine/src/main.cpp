...
public:
  ...
  virtual void onRender() override {
    ...
    ImGui::DragInt("Ray Depth",
                   const_cast<int *>(&m_renderer.getSettings().maxBounces), 1,
                   1, 500);

    if (ImGui::DragFloat("FOV", const_cast<float *>(&m_camera.getFov()), 1.0f,
                         179.0f)) {
      m_camera = Camera(m_camera.getFov(), 0.1f, 100.0f, m_camera.getPosition(),
                        m_camera.getDirection());
      m_scene.camera = m_camera;
      m_renderer.resetFrameIndex();
    }
    ...
    static std::function<void(const std::shared_ptr<SceneNode> &)> drawNode =
        [&](const std::shared_ptr<SceneNode> &node) {
          ...
          if (ImGui::TreeNodeEx(node->getName().c_str(), flags)) {
            ...
            if (ImGui::DragFloat3("Scale",
                                  const_cast<float *>(&node->getScale()[0]),
                                  0.01f, 0.0f, FLT_MAX)) {
            ...
            if (!node->getSpheres().empty()) {
              ...
              for (size_t i = 0; i < node->getSpheres().size(); i++) {
                ...
                if (ImGui::CollapsingHeader(sphereLabel.c_str(),
                                            ImGuiTreeNodeFlags_DefaultOpen)) {
                  if (ImGui::DragFloat3(
                          "Center",
                          const_cast<float *>(&node->getSpheres()[i].center[0]),
                          0.01f))
                    m_renderer.resetFrameIndex();
                  if (ImGui::DragFloat(
                          "Radius",
                          const_cast<float *>(&node->getSpheres()[i].radius),
                          0.01f))
                    m_renderer.resetFrameIndex();
                  if (ImGui::Combo("Material",
                                   const_cast<int *>(
                                       &node->getSpheres()[i].materialIndex),
                                   "Material 1\0Material 2\0Material 3\0\0"))
                    m_renderer.resetFrameIndex();
                }
                ImGui::PopID();
              }
    ...
    for (size_t i = 0; i < m_scene.materials.size(); i++) {
      ...
      ImGui::ColorEdit3(
          "Albedo", reinterpret_cast<float *>(&m_scene.materials[i].albedo));
      ImGui::DragFloat("Roughness", &m_scene.materials[i].roughness, 0.01f,
                       0.0f, 1.0f);
      ImGui::DragFloat("Metallic", &m_scene.materials[i].metallic, 0.01f, 0.0f,
                       1.0f);
    ...
    for (size_t i = 0; i < m_scene.lights.size(); i++) {
      ...
      ImGui::ColorEdit3("Color",
                        reinterpret_cast<float *>(&m_scene.lights[i].color));
      ImGui::DragFloat("Intensity", &m_scene.lights[i].intensity, 0.01f, 0.0f,
                       FLT_MAX);
