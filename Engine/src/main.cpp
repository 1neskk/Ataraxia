...
class Ataraxia final : public Layer {
   public:
    Ataraxia() : m_camera(45.0f, 0.1f, 100.0f) {
    ...
    virtual void onRender() override {
        ...
        ImGui::Checkbox(
            "Accumulation",
            const_cast<bool *>(&m_renderer.getSettings().accumulation));
        if (ImGui::Button("Reset Frame Index")) m_renderer.resetFrameIndex();

        ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 10);
        ImGui::Checkbox("Sky Light",
                        const_cast<bool *>(&m_renderer.getSettings().skyLight));
        ImGui::DragInt("Ray Depth",
                       const_cast<int *>(&m_renderer.getSettings().maxBounces),
                       1, 1, 500);

        if (ImGui::DragFloat("FOV", const_cast<float *>(&m_camera.getFov()),
                             1.0f, 179.0f)) {
            m_camera = Camera(m_camera.getFov(), 0.1f, 100.0f,
                              m_camera.getPosition(), m_camera.getDirection());
            m_scene.camera = m_camera;
            m_renderer.resetFrameIndex();
        }
        ...
        ImGui::Separator();
        ImGui::Text("Last Render Time: %.3fms | (%.1f FPS)", m_lastRenderTime,
                    io.Framerate);

        ImGui::End();

        ImGui::Begin("Hierarchy");
        static std::function<void(const std::shared_ptr<SceneNode> &)>
            drawNode = [&](const std::shared_ptr<SceneNode> &node) {
        ...
        for (size_t i = 0; i < m_scene.materials.size(); i++) {
            ...
            ImGui::ColorEdit3("Albedo", reinterpret_cast<float *>(
                                            &m_scene.materials[i].albedo));
            ImGui::DragFloat("Roughness", &m_scene.materials[i].roughness,
                             0.01f, 0.0f, 1.0f);
            ImGui::DragFloat("Metallic", &m_scene.materials[i].metallic, 0.01f,
                             0.0f, 1.0f);
            ImGui::DragFloat3("F0", &m_scene.materials[i].F0[0], 0.01f, 0.0f,
                              1.0f);
        ...
        for (size_t i = 0; i < m_scene.lights.size(); i++) {
            ...
            ImGui::DragFloat3("Position", &m_scene.lights[i].position[0],
                              0.01f);
            ImGui::ColorEdit3(
                "Color", reinterpret_cast<float *>(&m_scene.lights[i].color));
            ImGui::DragFloat("Intensity", &m_scene.lights[i].intensity, 0.01f,
                             0.0f, FLT_MAX);
   ...
   private:
    ...
    void initializeScene() {
        ...
        std::shared_ptr<SceneNode> childNode1 =
            std::make_shared<SceneNode>("ChildNode1");
        ...
        std::shared_ptr<SceneNode> grandChildNode =
            std::make_shared<SceneNode>("GrandChildNode");
    ...
    app->setMenubarCallback([app]() {
        if (ImGui::BeginMenu("Add")) {
            if (ImGui::MenuItem("Sphere")) {
                ...
                layer->GetScene().rootNode->addSphere(
                    Sphere(glm::vec3(0.0f), 1.0f, 0));
            }
            ImGui::EndMenu();
        }
