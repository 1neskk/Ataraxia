...
{
    glm::vec3 aabbMin;
    uint32_t leftFirst; // If leaf: index to first primitive. If internal: index to left child.
    glm::vec3 aabbMax;
    uint32_t count; // If > 0, it's a leaf and this is primitive count. If 0, it's internal.
};
